...
export class FHIRXMLExportService {
  ...
  public questionnaireToXML(questionnaire: FHIRQuestionnaire): string {
    ...
    if (questionnaire.publisher) {
      xml += `  <publisher value="${this.escapeXML(
        questionnaire.publisher
      )}" />\n`;
    }

    if (questionnaire.description) {
      xml += `  <description value="${this.escapeXML(
   ...
   * @returns XML string representation
   */
  public questionnaireResponseToXML(
    ...
    xml += `  <questionnaire value="${this.escapeXML(
   ...
   */
  public combinedQuestionnaireResponseToXML(
    questionnaire: FHIRQuestionnaire,
    response: FHIRQuestionnaireResponse
  ): string {
    let xml = '<?xml version="1.0" encoding="UTF-8"?>\n';
    ...
    xml += '    <resource>\n';
    xml +=
      this.questionnaireToXML(questionnaire)
        .split('\n')
        .map((line) => (line.startsWith('<?xml') ? '' : `      ${line}`))
    ...
    xml += '    <resource>\n';
    xml +=
      this.enhancedQuestionnaireResponseToXML(response, questionnaire)
        .split('\n')
        .map((line) => (line.startsWith('<?xml') ? '' : `      ${line}`))
        .filter((line) => line.trim())
        .join('\n') + '\n';
   ...
   */
  public enhancedQuestionnaireResponseToXML(
    response: FHIRQuestionnaireResponse,
    questionnaire: FHIRQuestionnaire
  ): string {
    let xml = '<?xml version="1.0" encoding="UTF-8"?>\n';
  ...
  private itemToXML(item: any, indentLevel: number): string {
    ...
    // Add tooltip extension if available
    if (item.showTooltip && item.tooltipText) {
      xml += `${indent}  <extension url="http://hl7.org/fhir/StructureDefinition/questionnaire-help">\n`;
      xml += `${indent}    <valueString value="${this.escapeXML(
        item.tooltipText
      )}" />\n`;
      ...
      item.answerOption.forEach((option: any) => {
        ...
        if (option.valueCoding) {
          ...
          if (option.valueCoding.system) {
            xml += `${indent}      <system value="${this.escapeXML(
              option.valueCoding.system
            )}" />\n`;
          }
          if (option.valueCoding.code) {
            xml += `${indent}      <code value="${this.escapeXML(
              option.valueCoding.code
            )}" />\n`;
          }
          if (option.valueCoding.display) {
            xml += `${indent}      <display value="${this.escapeXML(
  ...
  private responseItemToXML(
    ...
    // Skip items without answers (no actual response data)
    if (!item.answer || item.answer.length === 0) {
      // For groups, still process sub-items
      if (item.item && item.item.length > 0) {
        let xml = '';
        item.item.forEach((subItem: any) => {
          xml += this.responseItemToXML(
            subItem,
            indentLevel,
            questionnaireItems
          );
      ...
      // Skip this item entirely if it has no answers and no sub-items
      return '';
    }
    ...
    if (item.answer && item.answer.length > 0) {
      item.answer.forEach((answer: any) => {
        ...
        if (answer.valueString !== undefined) {
          ...
          xml += `${indent}    <valueDate value="${this.escapeXML(
            answer.valueDate
          )}" />\n`;
        } else if (answer.valueDateTime !== undefined) {
          xml += `${indent}    <valueDateTime value="${this.escapeXML(
            answer.valueDateTime
          )}" />\n`;
        } else if (answer.valueTime !== undefined) {
          xml += `${indent}    <valueTime value="${this.escapeXML(
            answer.valueTime
          )}" />\n`;
   ...
   */
  private enhancedResponseItemToXML(
    item: any,
    indentLevel: number,
    questionnaireItems?: any[]
  ): string {
    ...
    if (!item.answer || item.answer.length === 0) {
      ...
      if (item.item && item.item.length > 0) {
        let xml = '';
        item.item.forEach((subItem: any) => {
          xml += this.enhancedResponseItemToXML(
            subItem,
            indentLevel,
            questionnaireItems
          );
    ...
    // Find corresponding questionnaire item for tooltip text
    const questionnaireItem = this.findQuestionnaireItem(
      item.linkId,
      questionnaireItems
    );

    // Add tooltip text as extension if available
    if (questionnaireItem?.showTooltip && questionnaireItem?.tooltipText) {
      xml += `${indent}  <extension url="http://hl7.org/fhir/StructureDefinition/questionnaire-help">\n`;
      xml += `${indent}    <valueString value="${this.escapeXML(
        questionnaireItem.tooltipText
      )}" />\n`;
      xml += `${indent}  </extension>\n`;
    }
